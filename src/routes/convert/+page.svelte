<script lang="ts">
  ...
  async function probeFile(id: number, path: string) {
    markFileProbing(id);
    try {
      const info = await pluginInvoke<ProbeInfo>("convert", "probe_file", { path });
  ...
  async function loadHwAccel() {
    try {
      const info = await pluginInvoke<HwAccelInfo>("convert", "get_hwaccel_info");
      setHwAccel(info);
    } catch {
    }
  }
  ...
  async function startConversion() {
    ...
    for (const file of readyFiles) {
      ...
      try {
        const conversionId = await pluginInvoke<number>("convert", "convert_file", { options: convOptions });
...
{:else if pluginStatus === "not-installed"}
  <div class="plugin-guard">
    <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M20 10H4l4-4" /><path d="M4 14h16l-4 4" />
    </svg>
