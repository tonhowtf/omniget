<script lang="ts">
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("voomp_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("voomp_login_token", { token: token.trim() });
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("voomp_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: VoompCourse) {
    ...
    try {
      await invoke("start_voomp_course_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("voomp_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
