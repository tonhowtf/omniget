<script lang="ts">
  ...
  let courses: GranCursosCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("grancursos_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("grancursos_login_token", { token: token.trim() });
  ...
  async function handleLogout() {
    try {
      await invoke("grancursos_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("grancursos_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: GranCursosCourse) {
    ...
    try {
      await invoke("start_grancursos_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("grancursos_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
