<script lang="ts">
  ...
  let courses: GreennCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("greenn_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("greenn_login_token", { token: token.trim() });
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("greenn_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: GreennCourse) {
    ...
    try {
      await invoke("start_greenn_course_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("greenn_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
    ...
    {:else}
      ...
      <div class="courses-grid">
        {#each paginatedCourses as course (course.id)}
          <CourseCard
            name={course.name}
            price={course.lessons_count === 1 ? "1 lesson" : `${course.lessons_count} lessons`}
