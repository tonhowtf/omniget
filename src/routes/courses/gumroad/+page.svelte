<script lang="ts">
  ...
  let courses: GumroadProduct[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await pluginInvoke<string>("courses", "gumroad_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleLogin() {
    ...
    loading = true;
    try {
      const result = await pluginInvoke<string>("courses", "gumroad_login", { email, password });
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await pluginInvoke<string>("courses", "gumroad_login_token", { token: token.trim() });
  ...
  async function handleLogout() {
    try {
      await pluginInvoke("courses", "gumroad_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await pluginInvoke("courses", "gumroad_list_products");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(product: GumroadProduct) {
    const status = getCourseDownloadStatus(Number(product.id) || 0);
    ...
    try {
      await pluginInvoke("courses", "start_gumroad_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await pluginInvoke("courses", "gumroad_refresh_products");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
  ...
  <div class="page-logged">
    ...
    {:else}
      ...
      <div class="courses-grid">
        {#each paginatedCourses as product (product.id)}
          <CourseCard
            name={product.name}
            price={product.creator_name}
            imageUrl={product.preview_url ?? undefined}
            downloadStatus={getCourseDownloadStatus(Number(product.id) || 0)}
            downloadPercent={getCourseDownloadPercent(Number(product.id) || 0)}
            onDownload={() => downloadCourse(product)}
          />
        {/each}
      </div>
