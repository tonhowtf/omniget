<script lang="ts">
  ...
  let courses: MasterClassCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      await invoke<string>("masterclass_check_session");
      loggedIn = true;
      loadCourses();
    } catch {
      loggedIn = false;
  ...
  async function handleCookieLogin() {
    ...
    loading = true;
    try {
      await invoke<string>("masterclass_login_cookies", { cookies: cookies.trim() });
      loggedIn = true;
      loadCourses();
    } catch (e: any) {
  ...
  async function handleLogout() {
    try {
      await invoke("masterclass_logout");
    } catch {
    }
    loggedIn = false;
    courses = [];
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("masterclass_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: MasterClassCourse) {
    ...
    try {
      await invoke("start_masterclass_course_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("masterclass_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
...
{:else if loggedIn}
  <div class="page-logged">
    <div class="session-bar">
      <span class="session-info">MasterClass</span>
    ...
    {:else}
      ...
      <div class="courses-grid">
        {#each paginatedCourses as course (course.id)}
          <CourseCard
            name={course.instructor ? `${course.instructor} - ${course.name}` : course.name}
            price=""
  ...
  <div class="page-center">
    <div class="login-card">
      ...
      <div class="form">
        ...
        <button class="button" onclick={handleCookieLogin} disabled={loading || !cookies.trim()}>
          {#if loading}
