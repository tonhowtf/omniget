<script lang="ts">
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("thinkific_check_session");
  ...
  async function handleLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("thinkific_login", {
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("thinkific_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: ThinkificCourse) {
    ...
    try {
      await invoke("start_thinkific_course_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("thinkific_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
