<script lang="ts">
  ...
  type EstrategiaConcursosCourse = {
    id: number;
    name: string;
    total_aulas: number;
  };
  ...
  let courses: EstrategiaConcursosCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("estrategia_concursos_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("estrategia_concursos_login_token", { token: token.trim() });
  ...
  async function handleLogout() {
    try {
      await invoke("estrategia_concursos_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("estrategia_concursos_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: EstrategiaConcursosCourse) {
    ...
    try {
      await invoke("start_estrategia_concursos_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("estrategia_concursos_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
    ...
    {:else}
      ...
      <div class="courses-grid">
        {#each paginatedCourses as course (course.id)}
          <CourseCard
            name={course.name}
            price={`${course.total_aulas} aulas`}
...
{:else}
  <div class="page-center">
    <div class="login-card">
      <h2>Estrat&eacute;gia Concursos</h2>
