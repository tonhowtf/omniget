<script lang="ts">
  ...
  let loginMode = $state<"credentials" | "cookie">("credentials");
  ...
  function onFileSelected(e: Event) {
    ...
    reader.onload = () => { cookie = reader.result as string; };
    reader.readAsText(file);
    input.value = "";
  }

  let courses: MemberkitCourse[] = $state([]);
  ...
  async function downloadCourse(course: MemberkitCourse) {
  ...
  <div class="page-center">
    <div class="login-card">
      ...
      <div class="login-tabs">
        ...
        <button
          class="button login-tab"
          class:active={loginMode === "cookie"}
          onclick={() => { loginMode = "cookie"; error = ""; }}
        >
          Cookie
        </button>
      </div>
      ...
      {#if loginMode === "credentials"}
        <form class="form" onsubmit={(e) => { e.preventDefault(); handleLogin(); }}>
          <label class="field">
            ...
            <input
              type="url"
              placeholder="https://example.memberkit.com.br"
              bind:value={siteUrl}
              class="input"
              disabled={loading}
              required
      ...
      {:else}
        <div class="form">
          ...
          <button class="button" onclick={handleCookieLogin} disabled={loading || !cookie.trim()}>
            {#if loading}
              {$t('hotmart.authenticating')}
            {:else}
              {$t('hotmart.login')}
