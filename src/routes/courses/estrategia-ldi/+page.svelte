<script lang="ts">
  ...
  let courses: EstrategiaLdiCourse[] = $state([]);
  ...
  async function checkSession() {
    try {
      const result = await invoke<string>("estrategia_ldi_check_session");
      sessionEmail = result;
      loggedIn = true;
      loadCourses();
    } catch {
  ...
  async function handleTokenLogin() {
    ...
    loading = true;
    try {
      const result = await invoke<string>("estrategia_ldi_login_token", { token: token.trim() });
  ...
  async function handleLogout() {
    try {
      await invoke("estrategia_ldi_logout");
    } catch {
    }
    loggedIn = false;
    sessionEmail = "";
  ...
  async function loadCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("estrategia_ldi_list_courses");
      currentPage = 1;
    } catch (e: any) {
  ...
  async function downloadCourse(course: EstrategiaLdiCourse) {
    ...
    try {
      await invoke("start_estrategia_ldi_download", {
  ...
  async function refreshCourses() {
    ...
    coursesError = "";
    try {
      courses = await invoke("estrategia_ldi_refresh_courses");
      currentPage = 1;
    } catch (e: any) {
      coursesError = typeof e === "string" ? e : e.message ?? $t('hotmart.courses_error');
    } finally {
