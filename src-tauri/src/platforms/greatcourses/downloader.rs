...
use super::api::{self, WondriumCourse, WondriumSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct WondriumCourseDownloadProgress {
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            if let Some(ref vid_id) = lesson.video_id {
                ...
                let video_url = api::get_video_url(vid_id);
                ...
                match download_with_ytdlp(&video_url, &mod_dir, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[wondrium] yt-dlp failed for '{}': {}, trying HLS",
                            lesson.name,
                            e
                        );
                        match download_hls_video(&video_url, &video_path, &cancel_token).await {
                            ...
                            Err(e2) => {
                                tracing::error!(
                                    "[wondrium] HLS download also failed for '{}': {}",
                                    lesson.name,
                                    e2
                                );
            ...
            if let Some(ref pdf_url) = lesson.guidebook_url {
                let pdf_path = format!(
                    "{}/{}. {}.pdf",
                    mod_dir,
                    li + 1,
                ...
                if !tokio::fs::try_exists(&pdf_path).await.unwrap_or(false) {
                    match download_file_direct(&session.client, pdf_url, &pdf_path, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[wondrium] PDF download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                        }
...
async fn download_with_ytdlp(
    ...
    let result = crate::core::ytdlp::download_video(
        ...
        None,
        None,
        Some("https://www.thegreatcoursesplus.com/"),
        cancel_token.clone(),
        None,
        8,
        false,
