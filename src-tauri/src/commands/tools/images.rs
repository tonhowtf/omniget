...
#[tauri::command]
pub async fn tool_upscale_run(app: tauri::AppHandle, opts: upscale::UpscaleOptions) -> Result<upscale::UpscaleResult, String> {
    upscale::run(opts, progress(&app)).await.map_err(err)
}

#[tauri::command]
pub async fn tool_resize(app: tauri::AppHandle, opts: image_resize::ResizeOptions) -> Result<image_resize::ResizeResult, String> {
...
#[tauri::command]
pub async fn tool_ocr_run(app: tauri::AppHandle, inputs: Vec<String>, langs: String) -> Result<Vec<omniget_core::core::tools::ocr::OcrResult>, String> {
    omniget_core::core::tools::ocr::run(&inputs, &langs, progress(&app)).await.map_err(err)
}
