...
use omniget_core::core::tools::instagram::{
    self as ig, analytics, follow, media, profile, publish, social, IgClient, Session,
};
use serde::Serialize;
...
pub(crate) fn load_client(slug: Option<&str>) -> Result<Arc<IgClient>, String> {
    ...
    let content = crate::cookies::storage::read_account_file(DOMAIN, slug_name)
        .map_err(|_| ig::IgError::NoSession.to_string())?;
...
pub fn tool_ig_accounts() -> Vec<IgAccount> {
    ...
    let Some(bucket) = registry.buckets.get(DOMAIN) else {
        return Vec::new();
    };
    bucket
        .accounts
        ...
        .map(|a| {
            let content =
                crate::cookies::storage::read_account_file(DOMAIN, &a.slug).unwrap_or_default();
...
async fn resolve_items(
    ...
    url: &str,
    flag: &AtomicBool,
    p: &omniget_core::core::tools::ProgressFn,
    job: &str,
) -> Result<Vec<media::MediaItem>, String> {
    match ig::parse_target(url) {
        ig::IgTarget::Post { shortcode } => Ok(vec![media::post_info(client, &shortcode)
            .await
            .map_err(err)?]),
        ig::IgTarget::Story { username, media_id } => {
            let user = profile::resolve_user(client, &username)
                .await
                .map_err(err)?;
        ...
        ig::IgTarget::Profile { username, tab } => {
            let user = profile::resolve_user(client, &username)
                ...
                .map_err(err)?;
            match tab.as_str() {
                "reels" => profile::user_reels(client, &user.pk, 24, flag, p, job)
                    .await
                    .map_err(err),
                "tagged" => profile::user_tagged(client, &user.pk, 24, flag, p, job)
                    .await
                    .map_err(err),
                _ => profile::user_posts(client, &user.username, 24, flag, p, job)
                    .await
                    .map_err(err),
            }
        }
        ig::IgTarget::Tag { name } => social::tag_media(client, &name, "recent", 24, flag, p, job)
            .await
            .map_err(err),
...
#[tauri::command]
pub async fn tool_ig_download(
    app: tauri::AppHandle,
    slug: Option<String>,
    items: Vec<media::MediaItem>,
    dest: String,
    opts: media::DownloadOptions,
    job: String,
) -> Result<media::DownloadResult, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    media::download_items(
        &client,
        &items,
        &dest,
        &opts,
...
#[tauri::command]
pub async fn tool_ig_download_bulk(
    app: tauri::AppHandle,
    slug: Option<String>,
    urls: Vec<String>,
    dest: String,
    ...
    for (i, url) in urls.iter().enumerate() {
        ...
        omniget_core::core::tools::report(
            &p,
            &format!("ig:{}", job),
            "resolve",
            i as u64,
    ...
    let result = media::download_items(&client, &items, &dest, &opts, &p, &job, &j.flag)
        .await
        .map_err(err)?;
    Ok(BulkResult {
        items: items.len(),
...
#[tauri::command]
pub async fn tool_ig_profile_media(
    app: tauri::AppHandle,
    slug: Option<String>,
    user: String,
    tab: String,
    ...
    let p = progress(&app);
    if tab == "saved" {
        return profile::saved(&client, limit, &j.flag, &p, &job)
            .await
            .map_err(err);
    }
    let info = profile::resolve_user(&client, &user).await.map_err(err)?;
    match tab.as_str() {
        "reels" => profile::user_reels(&client, &info.pk, limit, &j.flag, &p, &job)
            .await
            .map_err(err),
        "tagged" => profile::user_tagged(&client, &info.pk, limit, &j.flag, &p, &job)
            .await
            .map_err(err),
        ...
        "highlights" => {
            ...
            for (i, h) in hls.iter().enumerate() {
                ...
                out.extend(
                    profile::highlight_items(&client, &h.id)
                        .await
                        .map_err(err)?,
                );
                client.pause().await;
        ...
        _ => profile::user_posts(&client, &info.username, limit, &j.flag, &p, &job)
            .await
            .map_err(err),
    }
}
...
#[tauri::command]
pub async fn tool_ig_stories(
    slug: Option<String>,
    user: String,
) -> Result<Vec<profile::Reel>, String> {
    let client = load_client(slug.as_deref())?;
    let info = profile::resolve_user(&client, &user).await.map_err(err)?;
...
#[tauri::command]
pub async fn tool_ig_highlights(
    slug: Option<String>,
    user: String,
) -> Result<Vec<profile::Highlight>, String> {
    let client = load_client(slug.as_deref())?;
    let info = profile::resolve_user(&client, &user).await.map_err(err)?;
...
#[tauri::command]
pub async fn tool_ig_highlight_items(
    slug: Option<String>,
    id: String,
) -> Result<Vec<media::MediaItem>, String> {
    let client = load_client(slug.as_deref())?;
...
#[tauri::command]
pub async fn tool_ig_story_viewers(
    slug: Option<String>,
    story_pk: String,
    job: String,
) -> Result<StoryViewers, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let (total, viewers) = profile::story_viewers(&client, &story_pk, &j.flag)
        .await
        .map_err(err)?;
...
#[tauri::command]
pub async fn tool_ig_follow_lists(
    app: tauri::AppHandle,
    slug: Option<String>,
    user: Option<String>,
    limit: usize,
    job: String,
) -> Result<follow::FollowAnalysis, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let p = progress(&app);
    ...
    let followers = follow::followers(&client, &target, limit, &j.flag, &p, &job)
        .await
        .map_err(err)?;
    let following = follow::following(&client, &target, limit, &j.flag, &p, &job)
        .await
        .map_err(err)?;
...
#[tauri::command]
pub fn tool_ig_whitelist_set(
    slug: Option<String>,
    users: Vec<follow::MiniUser>,
) -> Result<follow::Whitelist, String> {
    let client = load_client(slug.as_deref())?;
...
#[tauri::command]
pub async fn tool_ig_actions(
    app: tauri::AppHandle,
    slug: Option<String>,
    action: String,
    users: Vec<follow::MiniUser>,
    pacing: Option<follow::Pacing>,
    job: String,
) -> Result<follow::ActionReport, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    Ok(follow::run_actions(
        &client,
        &action,
        users,
        &pacing.unwrap_or_default(),
        &j.flag,
        &progress(&app),
        &job,
    )
...
#[tauri::command]
pub async fn tool_ig_resolve_users(
    slug: Option<String>,
    usernames: Vec<String>,
    job: String,
) -> Result<Vec<follow::MiniUser>, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let mut out = Vec::new();
    for u in usernames {
...
#[tauri::command]
pub async fn tool_ig_snapshot_take(
    app: tauri::AppHandle,
    slug: Option<String>,
    job: String,
) -> Result<follow::SnapshotMeta, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let p = progress(&app);
    let owner = owner_of(&client);
    let followers = follow::followers(&client, &owner, 0, &j.flag, &p, &job)
        .await
        .map_err(err)?;
    let following = follow::following(&client, &owner, 0, &j.flag, &p, &job)
        .await
        .map_err(err)?;
    if ig::cancelled(&j.flag) {
...
#[tauri::command]
pub async fn tool_ig_ghosts(
    app: tauri::AppHandle,
    slug: Option<String>,
    posts_limit: usize,
    comment_pages: usize,
    job: String,
) -> Result<analytics::GhostReport, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let p = progress(&app);
    let me = profile::whoami(&client).await.map_err(err)?;
    let followers = follow::followers(&client, &me.pk, 0, &j.flag, &p, &job)
        .await
        .map_err(err)?;
    let posts = profile::user_posts(&client, &me.username, posts_limit.max(1), &j.flag, &p, &job)
        .await
        .map_err(err)?;
    analytics::ghosts(&client, followers, &posts, comment_pages, &j.flag, &p, &job)
        .await
        .map_err(err)
}
...
pub async fn tool_ig_export(path: String) -> Result<analytics::ExportReport, String> {
    tokio::task::spawn_blocking(move || analytics::analyze_export(&path))
        .await
        .map_err(err)?
        .map_err(err)
}
...
#[tauri::command]
pub async fn tool_ig_analytics(
    app: tauri::AppHandle,
    slug: Option<String>,
    users: Vec<String>,
    posts_limit: usize,
    job: String,
) -> Result<Vec<analytics::ProfileStats>, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let p = progress(&app);
    let mut out = Vec::new();
    for u in users.iter().filter(|u| !u.trim().is_empty()) {
        ...
        let posts = match profile::user_posts(
            &client,
            &info.username,
            posts_limit.max(1),
            &j.flag,
...
pub async fn tool_ig_hashtag(
    ...
    limit: usize,
    job: String,
) -> Result<HashtagResult, String> {
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let info = social::tag_info(&client, &tag).await.map_err(err)?;
    let items = social::tag_media(&client, &tag, &tab, limit, &j.flag, &progress(&app), &job)
        .await
        .unwrap_or_default();
...
pub async fn tool_ig_comments(
    ...
    let client = load_client(slug.as_deref())?;
    let j = Job::new(&job);
    let ig::IgTarget::Post { shortcode } = ig::parse_target(&url) else {
        return Err("cole o link de um post ou reel".into());
    };
    let item = media::post_info(&client, &shortcode).await.map_err(err)?;
    let comments = social::comments(&client, &item.pk, limit, &j.flag, &progress(&app), &job)
        .await
        .map_err(err)?;
...
#[tauri::command]
pub fn tool_ig_giveaway(
    comments: Vec<social::Comment>,
    rules: social::GiveawayRules,
) -> social::GiveawayResult {
...
#[tauri::command]
pub async fn tool_ig_publish(
    app: tauri::AppHandle,
    slug: Option<String>,
    request: publish::PublishRequest,
    job: String,
) -> Result<publish::PublishResult, String> {
    let client = load_client(slug.as_deref())?;
    let _j = Job::new(&job);
    publish::publish_web(&client, &request, &progress(&app), &job)
        .await
        .map_err(err)
}

#[tauri::command]
pub async fn tool_ig_publish_graph(
    app: tauri::AppHandle,
    auth: publish::GraphAuth,
    request: publish::PublishRequest,
    job: String,
) -> Result<publish::PublishResult, String> {
    let _j = Job::new(&job);
    publish::publish_graph(&auth, &request, &progress(&app), &job)
        .await
        .map_err(err)
}
...
#[tauri::command]
pub fn tool_ig_schedule_add(
    app: tauri::AppHandle,
    post: publish::ScheduledPost,
) -> Result<publish::ScheduleStore, String> {
    start_scheduler(app);
