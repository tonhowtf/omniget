...
use crate::platforms::eduzznutror::api::{self, NutrorCourse, NutrorSession};
use crate::platforms::eduzznutror::downloader;
use crate::AppState;
...
struct NutrorDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn nutror_login_token(
    state: tauri::State<'_, AppState>,
    token: String,
    refresh_token: Option<String>,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    state.nutror_session.lock().await.take();
    ...
    *state.nutror_courses_cache.lock().await = None;
    ...
    match api::validate_token(&session).await {
        Ok(true) => {
            ...
            let mut guard = state.nutror_session.lock().await;
            *guard = Some(session);
            *state.nutror_session_validated_at.lock().await = Some(Instant::now());
...
#[tauri::command]
pub async fn nutror_check_session(
    state: tauri::State<'_, AppState>,
) -> Result<String, String> {
    let has_memory_session = state.nutror_session.lock().await.is_some();
    ...
    let guard = state.nutror_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.nutror_session_validated_at.lock().await;
    ...
    match api::validate_token(&session_clone).await {
        ...
        Ok(false) => {
            ...
            if let Some(old_session) = session_clone2 {
                if old_session.refresh_token.is_some() {
                    match api::refresh_token(&old_session).await {
                        Ok(new_session) => {
                            let _ = api::save_session(&new_session).await;
                            let mut guard = state.nutror_session.lock().await;
...
async fn fetch_nutror_courses(
    state: &tauri::State<'_, AppState>,
) -> Result<Vec<NutrorCourse>, String> {
    let guard = state.nutror_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.nutror_courses_cache.lock().await;
    *cache = Some(crate::NutrorCoursesCache {
...
#[tauri::command]
pub async fn nutror_list_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<NutrorCourse>, String> {
    {
        let cache = state.nutror_courses_cache.lock().await;
    ...
    fetch_nutror_courses(&state).await
}

#[tauri::command]
pub async fn nutror_refresh_courses(
    state: tauri::State<'_, AppState>,
) -> Result<Vec<NutrorCourse>, String> {
    {
        let mut cache = state.nutror_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_nutror_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: NutrorCourse =
