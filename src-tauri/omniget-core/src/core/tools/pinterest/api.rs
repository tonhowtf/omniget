...
pub enum Target {
    Pin { id: String },
    Board { user: String, slug: String },
    Section { user: String, slug: String, section: String },
...
const RESERVED: &[&str] = &[
    "pin", "search", "ideas", "today", "settings", "login", "signup", "business", "explore", "categories", "topics",
    "news_hub", "videos", "shopping", "help", "about", "policy", "resource", "oauth", "password", "notifications",
    "messages", "homefeed", "create", "pin-builder", "pin-creation-tool", "board", "boards", "_created", "_saved",
];
...
pub fn parse_target(input: &str) -> Option<Target> {
    ...
    let Ok(u) = url::Url::parse(&with_scheme) else {
        return Some(Target::Search { query: s.to_string(), scope: "pins".into() });
    };
    let host = u.host_str().unwrap_or("").to_ascii_lowercase();
    if host == "pin.it" {
        let code = u.path().trim_matches('/').to_string();
        return if code.is_empty() { None } else { Some(Target::Short { code }) };
    }
    ...
    let segs: Vec<String> = u
        .path_segments()
        .map(|it| it.filter(|p| !p.is_empty()).map(|p| p.to_string()).collect())
        .unwrap_or_default();
    match segs.as_slice() {
        [] => None,
        [first, rest @ ..] if first == "pin" => {
            ...
            if id.is_empty() { None } else { Some(Target::Pin { id }) }
        }
        [first, scope, ..] if first == "search" => {
            let query = u.query_pairs().find(|(k, _)| k == "q").map(|(_, v)| v.to_string()).unwrap_or_default();
        ...
        [user] if !RESERVED.contains(&user.as_str()) => Some(Target::User { username: user.clone() }),
        [user, tab] if tab == "_created" => Some(Target::UserCreated { username: user.clone() }),
        [user, tab] if tab == "_saved" || tab == "pins" => Some(Target::User { username: user.clone() }),
        [user, slug] if !RESERVED.contains(&user.as_str()) => Some(Target::Board { user: user.clone(), slug: slug.clone() }),
...
#[serde(tag = "kind", rename_all = "snake_case")]
pub enum Feed {
    Board { board_id: String, include_sections: bool },
    Section { section_id: String },
    UserPins { username: String },
    UserCreated { username: String },
    Search { query: String, scope: String },
    Related { pin_id: String },
    BoardIdeas { board_id: String },
}
...
fn media(v: &Value) -> Option<Media> {
    let url = s(&v["url"])?;
    Some(Media { url, width: n(&v["width"]) as u32, height: n(&v["height"]) as u32 })
}
...
fn person(v: &Value) -> Option<Person> {
    ...
    Some(Person {
        ...
        name,
        avatar: s(&v["image_medium_url"]).or_else(|| s(&v["image_large_url"])).or_else(|| s(&v["image_small_url"])),
    })
}
...
fn largest_image(images: &Value) -> (Option<Media>, Option<Media>, Option<String>) {
    ...
    let orig = obj.get("orig").or_else(|| obj.get("originals")).and_then(media);
    ...
    let thumb = obj.get("236x").and_then(media).map(|m| m.url).or_else(|| large.as_ref().map(|m| m.url.clone()));
    (orig, large, thumb)
}
...
pub fn parse_pin(v: &Value) -> Option<Pin> {
    ...
    let title = s(&v["title"]).or_else(|| s(&v["grid_title"])).unwrap_or_default();
    let description = s(&v["description"])
        ...
        .unwrap_or_default();
    let alt_text = s(&v["alt_text"]).or_else(|| s(&v["auto_alt_text"])).or_else(|| s(&v["seo_alt_text"])).unwrap_or_default();
    ...
    if image.is_none() {
        if let (Some(sig), Some(l)) = (&image_signature, &image_large) {
            ...
            image = Some(Media {
                url: format!("https://i.pinimg.com/originals/{}/{}/{}/{}.{}", &sig[0..2], &sig[2..4], &sig[4..6], sig, ext),
                width: 0,
                height: 0,
            });
        }
            ...
            for block in blocks {
                ...
                if bt == 3 || btype.contains("video") {
                    if let Some(vv) = video_from_list(&block["video"]["video_list"]) {
                        ...
                        extras.push(Extra { index: idx, kind: "video".into(), image: None, video: Some(vv) });
                        idx += 1;
                    }
                } else if bt == 2 || btype.contains("image") {
                    let (o, l, _) = largest_image(&block["image"]["images"]);
                    let m = o.or(l).or_else(|| {
                        let sig = s(&block["image_signature"]).or_else(|| s(&page["image_signature"]))?;
                        Some(Media {
                            url: format!("https://i.pinimg.com/originals/{}/{}/{}/{}.jpg", &sig[0..2], &sig[2..4], &sig[4..6], sig),
                            width: 0,
                            height: 0,
                        })
                    });
                    if let Some(m) = m {
                        extras.push(Extra { index: idx, kind: "image".into(), image: Some(m), video: None });
                        idx += 1;
                    }
                }
            }
    ...
    if let Some(slots) = v["carousel_data"]["carousel_slots"].as_array() {
        ...
        for (i, slot) in slots.iter().enumerate() {
            ...
            let m = o.or_else(|| l.map(|m| Media { url: to_originals(&m.url), ..m }));
            if let Some(m) = m {
                extras.push(Extra { index: i, kind: "image".into(), image: Some(m), video: None });
            }
        }
    }

    let is_gif = image.as_ref().map(|m| m.url.to_lowercase().ends_with(".gif")).unwrap_or(false);
    ...
    let reactions = v["reaction_counts"].as_object().map(|o| o.values().map(n).sum()).unwrap_or(0);
    let rich_v = if v["rich_metadata"].is_object() { &v["rich_metadata"] } else { &v["rich_summary"] };
    ...
    let board = board_v.as_object().map(|_| BoardRef {
        id: s(&board_v["id"]),
        name: s(&board_v["name"]),
        url: s(&board_v["url"]).map(|u| if u.starts_with('/') { format!("{}{}", ROOT, u) } else { u }),
    });
    ...
    Some(Pin {
        ...
        reactions,
        is_promoted: v["is_promoted"].as_bool().unwrap_or(false) || v["is_downstream_promotion"].as_bool().unwrap_or(false),
...
fn parse_board(v: &Value) -> Option<Board> {
    let id = s(&v["id"])?;
    let url = s(&v["url"]).map(|u| if u.starts_with('/') { format!("{}{}", ROOT, u) } else { u }).unwrap_or_default();
    Some(Board {
        id,
        ...
        cover: s(&v["image_cover_hd_url"]).or_else(|| s(&v["image_cover_url"])).or_else(|| {
            v["images"]["474x"].as_array().and_then(|a| a.first()).and_then(|m| s(&m["url"]))
        }),
...
pub fn cookie_header(input: Option<&str>) -> Option<String> {
    ...
    if p.exists() && p.is_file() {
        ...
        return crate::platforms::cookie_provider::cookie_header_from_netscape_for_domain(&content, "pinterest.com")
            .or_else(|| crate::platforms::cookie_provider::cookie_header_from_netscape(&content));
    }
    if raw.contains('\t') {
        return crate::platforms::cookie_provider::cookie_header_from_netscape_for_domain(raw, "pinterest.com");
    }
    if raw.contains('=') {
...
impl PinClient {
    pub fn new(cookies: Option<&str>) -> anyhow::Result<Self> {
        ...
        headers.insert("X-Pinterest-PWS-Handler", HeaderValue::from_static("www/[username].js"));
        ...
        let csrf = cookie.as_ref().and_then(|c| {
            c.split(';').map(|p| p.trim()).find_map(|p| p.strip_prefix("csrftoken=").map(|v| v.to_string()))
        });
    ...
    async fn resource(&self, name: &str, options: Value, source_url: &str) -> anyhow::Result<(Value, Option<String>)> {
        ...
        loop {
            ...
            if (status.as_u16() == 429 || status.is_server_error()) && attempt < 4 {
                tokio::time::sleep(Duration::from_millis(800 * attempt as u64 * attempt as u64)).await;
                continue;
            }
            let body: Value = match resp.json().await {
                Ok(v) => v,
                Err(e) => return Err(anyhow!("Pinterest respondeu HTTP {} sem JSON ({})", status, e)),
            };
            ...
            if rr["status"].as_str() != Some("success") {
                ...
                let code = rr["error"]["http_status"].as_u64().unwrap_or(status.as_u16() as u64);
                if code == 404 {
                    return Err(anyhow!("nao encontrado no Pinterest (privado, removido ou URL errada): {}", msg));
                }
                if code == 401 || code == 403 {
                    return Err(anyhow!("o Pinterest negou o acesso ({}); para boards secretos informe os cookies", msg));
                }
    ...
    pub async fn pin(&self, id: &str) -> anyhow::Result<Pin> {
        let (data, _) = self.resource("Pin", json!({ "id": id, "field_set_key": "detailed" }), &format!("/pin/{}/", id)).await?;
    ...
    pub async fn board(&self, user: &str, slug: &str) -> anyhow::Result<Board> {
        ...
        let (data, _) = self
            .resource("Board", json!({ "username": user, "slug": slug, "field_set_key": "detailed" }), &src)
            .await?;
    ...
    pub async fn user(&self, username: &str) -> anyhow::Result<User> {
        let (data, _) = self
            .resource("User", json!({ "username": username, "field_set_key": "profile" }), &format!("/{}/", username))
            .await?;
    ...
    pub async fn user_boards(&self, username: &str) -> anyhow::Result<Vec<Board>> {
        ...
        loop {
            ...
            let (data, next) = self.resource("Boards", opts, &format!("/{}/_saved/", username)).await?;
            let before = out.len();
            for v in data.as_array().cloned().unwrap_or_default() {
    ...
    pub async fn feed_page(&self, feed: &Feed, bookmark: Option<&str>, page_size: u32) -> anyhow::Result<(Vec<Pin>, Option<String>, Vec<String>)> {
        let bm = bookmark.map(|b| json!([b])).unwrap_or_else(|| json!([]));
        let (name, opts, src) = match feed {
            Feed::Board { board_id, include_sections } => (
                "BoardFeed",
            ...
            Feed::Search { query, scope } => (
                ...
                format!("/search/{}/?q={}&rs=typed", scope, urlencoding::encode(query)),
            ),
            Feed::Related { pin_id } => (
                "RelatedPinFeed",
        ...
        let items: Vec<Value> = if let Some(a) = data.as_array() {
            ...
            if let Some(g) = data["rankedGuides"].as_array() {
                for x in g {
                    if let Some(t) = s(&x["term"]).or_else(|| s(&x["display"])).or_else(|| s(&x["name"])) {
                        guides.push(t);
                    }
                }
            }
    ...
    pub async fn collect(
        ...
        loop {
            let want = if limit == 0 { PAGE } else { ((limit - out.len()).min(PAGE as usize)).max(1) as u32 };
    ...
    pub async fn search_boards(&self, query: &str, limit: usize) -> anyhow::Result<Vec<Board>> {
        ...
        let mut bookmark: Option<String> = None;
        loop {
            let bm = bookmark.as_ref().map(|b| json!([b])).unwrap_or_else(|| json!([]));
            let (data, next) = self
                .resource(
                    "BaseSearch",
    ...
    pub async fn typeahead(&self, term: &str) -> anyhow::Result<Vec<String>> {
        let (data, _) = self
            .resource("AdvancedTypeahead", json!({ "pin_scope": "pins", "count": 20, "term": term }), "/")
            .await?;
        let mut out = Vec::new();
        for it in data["items"].as_array().cloned().unwrap_or_default() {
            if let Some(t) = s(&it["label"]).or_else(|| s(&it["title"])).or_else(|| s(&it["query"])).or_else(|| s(&it["term"])) {
                if !out.contains(&t) {
                    out.push(t);
                }
            }
    ...
    pub async fn resolve_short(&self, code: &str) -> anyhow::Result<String> {
        ...
        for _ in 0..3 {
            if parse_target(&loc).map(|t| !matches!(t, Target::Short { .. })).unwrap_or(false) && !loc.ends_with("pinterest.com/") {
                break;
            }
            let r = self.http.get(&loc).send().await?;
            match r.headers().get(reqwest::header::LOCATION).and_then(|v| v.to_str().ok()) {
    ...
    pub async fn unsave(&self, pin_id: &str) -> anyhow::Result<()> {
        let csrf = self.csrf.clone().ok_or_else(|| anyhow!("para desfazer saves informe os cookies da sua sessao (com csrftoken)"))?;
        ...
        if !status.is_success() || body["resource_response"]["status"].as_str() != Some("success") {
            let msg = body["resource_response"]["error"]["message"].as_str().unwrap_or("falhou");
            return Err(anyhow!("nao consegui desfazer o save de {} (HTTP {}): {}", pin_id, status, msg));
        }
        Ok(())
    }
    ...
    pub async fn feed_for(&self, target: &Target) -> anyhow::Result<(Feed, String)> {
        match target {
            Target::Board { user, slug } => {
                ...
                Ok((Feed::Board { board_id: b.id.clone(), include_sections: true }, b.name))
            }
            Target::Section { user, slug, section } => {
                ...
                let sec = secs
                    ...
                    .ok_or_else(|| anyhow!("secao '{}' nao encontrada no board {}", section, b.name))?;
                Ok((Feed::Section { section_id: sec.id }, format!("{} · {}", b.name, sec.title)))
            }
            Target::User { username } => Ok((Feed::UserPins { username: username.clone() }, username.clone())),
            Target::UserCreated { username } => Ok((Feed::UserCreated { username: username.clone() }, format!("{} (criados)", username))),
            Target::Search { query, scope } => Ok((Feed::Search { query: query.clone(), scope: scope.clone() }, query.clone())),
            Target::Pin { id } => Ok((Feed::Related { pin_id: id.clone() }, format!("parecidos com {}", id))),
            Target::Short { code } => {
                let url = self.resolve_short(code).await?;
                let t = parse_target(&url).ok_or_else(|| anyhow!("nao entendi o destino de pin.it/{}", code))?;
...
mod tests {
    ...
    #[test]
    fn parses_targets() {
        assert_eq!(parse_target("https://www.pinterest.com/pin/8373949304441833/"), Some(Target::Pin { id: "8373949304441833".into() }));
        assert_eq!(parse_target("https://br.pinterest.com/pin/mid-century--8373949304441833/"), Some(Target::Pin { id: "8373949304441833".into() }));
        assert_eq!(parse_target("8373949304441833"), Some(Target::Pin { id: "8373949304441833".into() }));
        assert_eq!(parse_target("pin.it/2u3CBgh"), Some(Target::Short { code: "2u3CBgh".into() }));
        assert_eq!(parse_target("https://www.pinterest.com/pinterest/home-decor/"), Some(Target::Board { user: "pinterest".into(), slug: "home-decor".into() }));
        assert_eq!(
            parse_target("https://www.pinterest.fr/pinterest/home-decor/living-room/"),
            Some(Target::Section { user: "pinterest".into(), slug: "home-decor".into(), section: "living-room".into() })
        );
        assert_eq!(parse_target("https://www.pinterest.com/pinterest/"), Some(Target::User { username: "pinterest".into() }));
        assert_eq!(parse_target("https://www.pinterest.com/pinterest/_created/"), Some(Target::UserCreated { username: "pinterest".into() }));
        assert_eq!(parse_target("https://www.pinterest.com/search/videos/?q=cats&rs=typed"), Some(Target::Search { query: "cats".into(), scope: "videos".into() }));
        assert_eq!(parse_target("mid century living room"), Some(Target::Search { query: "mid century living room".into(), scope: "pins".into() }));
    ...
    fn rewrites_to_originals() {
        ...
        assert_eq!(to_originals("https://example.com/a.jpg"), "https://example.com/a.jpg");
    }
    ...
    fn parses_pin_json() {
        ...
        assert_eq!(p.video.as_ref().unwrap().mp4.as_deref(), Some("https://v1.pinimg.com/videos/mc/720p/a.mp4"));
        assert_eq!(p.video.unwrap().hls.as_deref(), Some("https://v1.pinimg.com/videos/iht/hls/v2/a.m3u8"));
        assert_eq!(p.saves, 12);
        assert_eq!(p.reactions, 5);
        assert!(p.is_promoted);
        assert!(p.ai.labeled);
        assert_eq!(p.ai.keyword_level, 2);
        assert_eq!(p.board.unwrap().url.unwrap(), "https://www.pinterest.com/u/b/");
    }

    #[test]
    fn cookie_header_from_raw_string() {
        assert_eq!(cookie_header(Some(" csrftoken=abc; _pinterest_sess=xyz ")).as_deref(), Some("csrftoken=abc; _pinterest_sess=xyz"));
...
mod live {
    ...
    async fn live_board_pin_search() {
        ...
        let (pins, next, _) = c.feed_page(&Feed::Board { board_id: b.id.clone(), include_sections: true }, None, 10).await.unwrap();
        ...
        let (pins, _, guides) = c.feed_page(&Feed::Search { query: "mid century living room".into(), scope: "pins".into() }, None, 10).await.unwrap();
        assert!(pins.len() >= 5);
        assert!(!guides.is_empty(), "guides vazios");
        let (vids, _, _) = c.feed_page(&Feed::Search { query: "cats".into(), scope: "videos".into() }, None, 5).await.unwrap();
        assert!(vids.iter().any(|v| v.video.is_some()), "nenhum video");
        let rel = c.collect(&Feed::Related { pin_id: p.id.clone() }, 8, |_| {}).await.unwrap();
        ...
        let (bytes, ext) = super::super::media::fetch_image(&c, &img.url).await.unwrap();
        assert!(bytes.len() > 1000);
        println!("orig {} bytes ext {} ({}x{})", bytes.len(), ext, img.width, img.height);
        ...
        assert_eq!(pal.len(), 5);
        println!("palette {:?}", pal.iter().map(|s| s.hex.clone()).collect::<Vec<_>>());
    }
    ...
    async fn live_short_link() {
        let c = PinClient::new(None).unwrap();
        let (pins, _, _) = c.feed_page(&Feed::Search { query: "cats".into(), scope: "pins".into() }, None, 1).await.unwrap();
        let p = &pins[0];
