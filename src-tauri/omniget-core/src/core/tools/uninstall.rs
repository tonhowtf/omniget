...
fn mac_leftovers(app: &App) -> Vec<Leftover> {
    ...
    let stem = Path::new(&app.path)
        ...
        .map(|s| s.to_string_lossy().to_string())
        .unwrap_or_default();
    let needles: Vec<String> = [app.key.clone(), stem.clone(), app.name.clone()]
        .into_iter()
        .filter(|s| s.len() >= 3)
    ...
    for d in dirs {
        let Ok(rd) = std::fs::read_dir(lib.join(d)) else {
            continue;
        };
        for e in rd.flatten() {
    ...
    for (i, (root, admin)) in UNINSTALL_KEYS.iter().enumerate() {
        ...
        for key in super::win_registry::reg_query(root, true).await {
            ...
            if name.trim().is_empty()
                || get("SystemComponent") == "0x1"
                || !get("ParentKeyName").is_empty() && get("ReleaseType").contains("Update")
            {
                continue;
            }
            let unins = if get("QuietUninstallString").is_empty() {
                get("UninstallString")
            } else {
            ...
            let kb =
                u64::from_str_radix(get("EstimatedSize").trim_start_matches("0x"), 16).unwrap_or(0);
            let kind = if unins.to_ascii_lowercase().contains("msiexec") {
                "msi"
            } else {
                "exe"
            };
...
async fn linux_list(progress: &super::ProgressFn) -> Vec<App> {
    ...
    if let Ok(list) = run("snap", &["list"]).await {
        for l in list.lines().skip(1) {
            ...
            if cols.len() < 2
                ...
                || cols[0].starts_with("gnome-")
                || cols[0].starts_with("gtk-common")
            {
                continue;
            }
            out.push(App {
    ...
    if let Ok(list) = run(
        ...
        &[
            "-W",
            "-f=${Package}\t${Version}\t${Installed-Size}\t${Maintainer}\n",
        ],
    )
    .await
    {
        if let Ok(files) = run(
            ...
            &[
                "-lc",
                "dpkg -S /usr/share/applications/*.desktop 2>/dev/null | cut -d: -f1 | sort -u",
            ],
        )
        .await
        {
    ...
    .await
    {
        if let Ok(files) = run(
            ...
            &[
                "-lc",
                "rpm -qf /usr/share/applications/*.desktop 2>/dev/null | sort -u",
            ],
        )
        .await
        {
...
fn linux_leftovers(app: &App) -> Vec<Leftover> {
    let h = home();
    let mut out = Vec::new();
    let needles: Vec<String> = [app.key.clone(), app.name.clone()]
        .into_iter()
        .filter(|s| s.len() >= 3)
        .map(|s| s.to_lowercase())
        .collect();
...
pub async fn uninstall(app: &App, leftover_paths: &[String]) -> UninstallResult {
    ...
    let res: anyhow::Result<String> = if cfg!(target_os = "macos") {
        ...
        match app.kind.as_str() {
            ...
            .await
            .map(|_| "desinstalado".into()),
            "snap" => run("pkexec", &["snap", "remove", &app.key])
                .await
                .map(|_| "desinstalado".into()),
            "deb" => run("pkexec", &["apt-get", "remove", "-y", &app.key])
                .await
                .map(|_| "desinstalado".into()),
            "rpm" => run("pkexec", &["dnf", "remove", "-y", &app.key])
                .await
                .map(|_| "desinstalado".into()),
            _ => trash::delete(&app.path)
                .map(|_| "movido para a lixeira".to_string())
                .map_err(|e| anyhow!("{}", e)),
        }
    };
    match res {
...
mod live {
    ...
    async fn list_here() {
        ...
        if let Some(a) = apps.iter().find(|a| {
            a.name.to_lowercase().contains("spotify")
                || a.name.to_lowercase().contains("discord")
                || a.name.to_lowercase().contains("code")
        }) {
