...
pub fn state() -> RecordState {
    ...
    RecordState {
        ...
        output: s
            .as_ref()
            .and_then(|s| s.output.as_ref().map(|p| p.to_string_lossy().to_string())),
...
pub async fn sources() -> Vec<Source> {
    ...
    if cfg!(target_os = "macos") {
                ...
                if let Some(i) = l.find('[') {
                    if let Some(j) = l[i..].find(']') {
                        ...
                        if idx.chars().all(|c| c.is_ascii_digit())
                            && (kind == "audio" || name.starts_with("Capture screen"))
                        {
                            out.push(Source {
                                id: idx.to_string(),
                                name: name.to_string(),
        ...
        out.push(Source {
            id: "desktop".into(),
            name: "Área de trabalho (todas as telas)".into(),
            kind: "screen".into(),
        });
            ...
            .await
        {
            for l in String::from_utf8_lossy(&o.stdout).lines() {
                ...
                if cols.len() > 1 && cols[1].ends_with(".monitor") {
                    out.push(Source {
                        id: cols[1].to_string(),
                        name: format!("Áudio do sistema ({})", cols[1]),
                        kind: "audio".into(),
                    });
                }
            }
...
fn build_args(opts: &RecordOptions, target: &Path, ring: bool) -> Vec<String> {
    ...
    if cfg!(target_os = "macos") {
        a.extend([
            ...
            fps.clone(),
            "-capture_cursor".into(),
            if opts.cursor { "1".into() } else { "0".into() },
            "-pix_fmt".into(),
            "uyvy422".into(),
        ]);
        let screen = if opts.screen.is_empty() {
            "1".to_string()
        } else {
            opts.screen.clone()
        };
        let audio = if opts.audio.is_empty() {
            "none".to_string()
        } else {
            opts.audio.clone()
        };
        ...
        let mut input = if opts.screen.is_empty() {
            format!(
                "{}.0",
...
mod tests {
    ...
    fn args_shape() {
        let o: RecordOptions =
            serde_json::from_str(r#"{"screen":"","fps":30,"replay_seconds":30}"#).unwrap();
