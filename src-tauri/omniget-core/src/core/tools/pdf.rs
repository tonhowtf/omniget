...
unsafe fn sym<T: Copy>(lib: &libloading::Library, name: &[u8]) -> anyhow::Result<T> {
    Ok(*lib.get::<T>(name).map_err(|e| anyhow!("PDFium sem {}: {}", String::from_utf8_lossy(name), e))?)
}
...
fn api() -> anyhow::Result<&'static Api> {
    ...
    let path = crate::core::pdfium::resolve_path().ok_or_else(|| anyhow!("PDFium nao esta instalado (Ajustes → Dependencias → PDFium)"))?;
    let lib = unsafe { libloading::Library::new(&path) }.map_err(|e| anyhow!("nao carregou o PDFium em {}: {}", path.display(), e))?;
    let api = unsafe {
...
impl Document {
    fn open(api: &'static Api, path: &Path, password: Option<&str>) -> anyhow::Result<Self> {
        ...
        let doc = unsafe { (api.load_mem)(data.as_ptr() as *const c_void, data.len(), pw.as_ptr()) };
        if doc.is_null() {
        ...
        Ok(Document { api, doc, _data: data })
    }
    ...
    fn new(api: &'static Api) -> anyhow::Result<Self> {
        ...
        Ok(Document { api, doc, _data: Vec::new() })
    }
    ...
    fn to_bytes(&self) -> anyhow::Result<Vec<u8>> {
        let mut w = Writer { fw: FileWrite { version: 1, write_block }, buf: Vec::new() };
    ...
    fn meta(&self, tag: &str) -> Option<String> {
        ...
        let len = unsafe { (self.api.meta_text)(self.doc, tag.as_ptr(), std::ptr::null_mut(), 0) } as usize;
        if len < 4 {
            return None;
        }
        let mut buf = vec![0u8; len];
        unsafe { (self.api.meta_text)(self.doc, tag.as_ptr(), buf.as_mut_ptr() as *mut c_void, len as c_ulong) };
...
pub fn parse_ranges(spec: &str, total: usize) -> anyhow::Result<Vec<usize>> {
    ...
    for part in spec.split([',', ' ']).map(str::trim).filter(|p| !p.is_empty()) {
        if let Some((a, b)) = part.split_once('-') {
            let a: usize = if a.trim().is_empty() { 1 } else { a.trim().parse().map_err(|_| anyhow!("intervalo invalido: {}", part))? };
            let b: usize = if b.trim().is_empty() { total } else { b.trim().parse().map_err(|_| anyhow!("intervalo invalido: {}", part))? };
            if a == 0 || b == 0 || a > total || b > total {
                return Err(anyhow!("pagina fora do documento ({} paginas): {}", total, part));
            }
            if a <= b {
                out.extend(a..=b);
            } else {
            ...
            let n: usize = part.parse().map_err(|_| anyhow!("pagina invalida: {}", part))?;
            if n == 0 || n > total {
                return Err(anyhow!("pagina fora do documento ({} paginas): {}", total, n));
            }
            out.push(n);
        }
    }
...
fn stem(path: &Path) -> String {
    path.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_else(|| "documento".into())
}
...
fn unique(path: PathBuf) -> PathBuf {
    ...
    for i in 2..1000 {
        let cand = dir.join(if ext.is_empty() { format!("{} ({})", stem, i) } else { format!("{} ({}).{}", stem, i, ext) });
        if !cand.exists() {
            return cand;
        }
    }
...
fn encode(img: &image::RgbImage, format: &str, quality: u8) -> anyhow::Result<Vec<u8>> {
    ...
    if format == "png" {
        ...
        let mut enc = image::codecs::jpeg::JpegEncoder::new_with_quality(&mut buf, quality.clamp(10, 100));
...
async fn find_soffice() -> Option<PathBuf> {
    ...
    let candidates: &[&str] = if cfg!(target_os = "windows") {
        ...
        &["/usr/bin/soffice", "/usr/bin/libreoffice", "/snap/bin/libreoffice", "/var/lib/flatpak/exports/bin/org.libreoffice.LibreOffice"]
    };
...
pub async fn status() -> PdfStatus {
    ...
    PdfStatus {
        ...
        libreoffice: find_soffice().await.map(|p| p.to_string_lossy().to_string()),
...
pub fn merge(opts: &MergeOptions, progress: &super::ProgressFn) -> anyhow::Result<PdfOut> {
    ...
    for (i, input) in opts.inputs.iter().enumerate() {
        report(progress, "progress", i as u64, Some(total), Some(input.clone()));
    ...
    Ok(PdfOut { output: out.to_string_lossy().to_string(), pages: dest.pages(), bytes })
}
...
pub fn split(opts: &SplitOptions, progress: &super::ProgressFn) -> anyhow::Result<PdfOuts> {
    ...
    let groups: Vec<(String, Vec<usize>)> = match opts.mode.as_str() {
        "each" => (1..=total).map(|p| (format!("p{:03}", p), vec![p])).collect(),
        "every" => {
        ...
        "ranges" => opts
            ...
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .map(|s| parse_ranges(s, total).map(|pages| (s.replace(' ', "").replace(',', "_"), pages)))
            .collect::<anyhow::Result<Vec<_>>>()?,
        _ => vec![(opts.ranges.replace(' ', "").replace(',', "_"), parse_ranges(&opts.ranges, total)?)],
    };
    if groups.is_empty() {
    ...
    Ok(PdfOuts { outputs, pages: total })
}
...
pub fn render(opts: &RenderOptions, progress: &super::ProgressFn) -> anyhow::Result<Vec<String>> {
    ...
    for (i, p) in pages.iter().enumerate() {
        report(progress, "progress", i as u64, Some(n), Some(format!("pagina {}", p)));
...
pub fn to_text(input: &str, pages: &str, save: bool, output_dir: &str) -> anyhow::Result<TextResult> {
    let api = api()?;
    let _g = OPS.lock().unwrap_or_else(|p| p.into_inner());
    ...
    Ok(TextResult { text, output, pages: pages.len() })
}

// ── Imagens → PDF ──────────────────────────────────────────────────────

pub fn images_to_pdf(inputs: &[String], output: &str, quality: u8, progress: &super::ProgressFn) -> anyhow::Result<PdfOut> {
    if inputs.is_empty() {
    ...
    for (i, input) in inputs.iter().enumerate() {
        ...
        if jpeg_pdf::is_jpeg(&data) {
            ...
            let img = image::load_from_memory(&data).map_err(|e| anyhow!("{}: {}", input, e))?.to_rgb8();
    ...
    report(progress, "done", n, Some(n), None);
    Ok(PdfOut { output: out.to_string_lossy().to_string(), pages: jpegs.len(), bytes: pdf.len() as u64 })
}

// ── Rasterizar de volta para PDF (comprimir e sanitizar) ───────────────

fn rasterize(input: &Path, output: &Path, dpi: u32, quality: u8, progress: &super::ProgressFn) -> anyhow::Result<PdfOut> {
    let api = api()?;
    let _g = OPS.lock().unwrap_or_else(|p| p.into_inner());
    let doc = Document::open(api, input, None)?;
    let total = doc.pages();
    ...
    for i in 0..total {
        report(progress, "progress", i as u64, Some(total as u64), Some(format!("pagina {}", i + 1)));
    ...
    Ok(PdfOut { output: output.to_string_lossy().to_string(), pages: total, bytes: pdf.len() as u64 })
}
...
pub async fn compress(opts: CompressOptions, progress: super::ProgressFn) -> anyhow::Result<CompressResult> {
    let input = PathBuf::from(opts.input.trim());
    let before = std::fs::metadata(&input)?.len();
    let output = unique(out_dir_for(&input, &opts.output_dir).join(format!("{} (comprimido).pdf", stem(&input))));
    let gs = if opts.mode == "raster" { None } else { find_gs().await };
    if let Some(gs) = gs {
        ...
        let o = crate::core::process::command(&gs)
            .args(["-sDEVICE=pdfwrite", "-dCompatibilityLevel=1.5", "-dNOPAUSE", "-dQUIET", "-dBATCH", "-dDetectDuplicateImages=true"])
        ...
        if o.status.success() && output.exists() {
            ...
            let pages = tokio::task::spawn_blocking({
                ...
                move || info(&out.to_string_lossy(), None).map(|i| i.pages).unwrap_or(0)
            })
            .await
            .unwrap_or(0);
            report(&progress, "done", 1, Some(1), None);
            return Ok(CompressResult { output: output.to_string_lossy().to_string(), before, after, method: "ghostscript".into(), pages });
        }
        if opts.mode == "gs" {
            return Err(anyhow!("ghostscript falhou: {}", String::from_utf8_lossy(&o.stderr).trim()));
        }
    ...
    let out = tokio::task::spawn_blocking(move || rasterize(&input, &output, dpi, quality, &progress)).await??;
    Ok(CompressResult { output: out.output, before, after: out.bytes, method: "raster".into(), pages: out.pages })
}
...
pub fn sanitize(input: &str, output_dir: &str, dpi: u32, quality: u8, progress: &super::ProgressFn) -> anyhow::Result<PdfOut> {
    let input = PathBuf::from(input.trim());
    let output = unique(out_dir_for(&input, output_dir).join(format!("{} (seguro).pdf", stem(&input))));
...
pub async fn ocr(input: String, langs: String, output_dir: String, dpi: u32, progress: super::ProgressFn) -> anyhow::Result<PdfOut> {
    let tesseract = super::ocr::locate().await.ok_or_else(|| anyhow!("tesseract nao esta instalado"))?;
    let input_path = PathBuf::from(input.trim());
    let langs = if langs.trim().is_empty() { "eng".to_string() } else { langs.trim().to_string() };
    ...
    let (pngs, pages) = {
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<(Vec<PathBuf>, usize)> {
            ...
            for i in 0..total {
                report(&progress, "render", i as u64, Some(total as u64), Some(format!("pagina {}", i + 1)));
                let img = doc.page(i)?.render(dpi)?;
    ...
    std::fs::write(&list, pngs.iter().map(|p| p.to_string_lossy().to_string()).collect::<Vec<_>>().join("\n"))?;
    ...
    if !o.status.success() {
        ...
        return Err(anyhow!("tesseract falhou: {}", String::from_utf8_lossy(&o.stderr).trim()));
    }
    let produced = base.with_extension("pdf");
    let output = unique(out_dir_for(&input_path, &output_dir).join(format!("{} (OCR).pdf", stem(&input_path))));
    if let Some(parent) = output.parent() {
        std::fs::create_dir_all(parent)?;
    }
    ...
    Ok(PdfOut { output: output.to_string_lossy().to_string(), pages, bytes })
}

// ── LibreOffice (PDF ↔ Office) ─────────────────────────────────────────

pub async fn office_convert(inputs: Vec<String>, target: String, output_dir: String, progress: super::ProgressFn) -> anyhow::Result<Vec<String>> {
    let soffice = find_soffice().await.ok_or_else(|| anyhow!("LibreOffice nao encontrado"))?;
    let target = target.trim().trim_start_matches('.').to_ascii_lowercase();
    if !matches!(target.as_str(), "docx" | "odt" | "pptx" | "xlsx" | "pdf" | "html" | "txt" | "epub") {
    ...
    for (i, input) in inputs.iter().enumerate() {
        ...
        std::fs::create_dir_all(&dir)?;
        let is_pdf = input_path.extension().map(|e| e.eq_ignore_ascii_case("pdf")).unwrap_or(false);
        ...
        cmd.arg("--convert-to").arg(&target).arg("--outdir").arg(&dir).arg(&input_path);
...
mod tests {
    ...
    async fn live() {
        ...
        for i in 0..3u8 {
            let img = image::RgbImage::from_fn(200, 300, |x, _| image::Rgb([x as u8, 80 + i * 40, 200]));
        ...
        assert_eq!(i.pages, 3);
        assert!(!i.has_text);
        let merged = merge(&MergeOptions { inputs: vec![a.to_string_lossy().to_string(), a.to_string_lossy().to_string()], output: dir.join("m.pdf").to_string_lossy().to_string() }, &p).unwrap();
        assert_eq!(merged.pages, 6);
        let sp = split(&SplitOptions { input: merged.output.clone(), mode: "every".into(), every: 4, ranges: String::new(), output_dir: String::new() }, &p).unwrap();
        assert_eq!(sp.outputs.len(), 2);
        assert_eq!(info(&sp.outputs[1], None).unwrap().pages, 2);
        let imgs = render(&RenderOptions { input: merged.output.clone(), pages: "1,6".into(), dpi: 50, format: "png".into(), quality: 0, output_dir: String::new() }, &p).unwrap();
        assert_eq!(imgs.len(), 2);
        let png = image::open(&imgs[0]).unwrap().to_rgb8();
        assert!(png.width() > 100 && png.get_pixel(png.width() - 1, 10)[2] > 150, "render azul");
        let t = to_text(&merged.output, "", false, "").unwrap();
        assert_eq!(t.pages, 6);
        let s = sanitize(&merged.output, "", 40, 50, &p).unwrap();
        assert_eq!(s.pages, 6);
        let c = compress(CompressOptions { input: merged.output.clone(), output_dir: String::new(), mode: "raster".into(), preset: String::new(), dpi: 30, quality: 30 }, p.clone()).await.unwrap();
        assert!(c.after < c.before, "{} < {}", c.after, c.before);
        println!("ok: {} {} {:?} {}", merged.output, sp.outputs[0], imgs, c.output);
        let _ = std::fs::remove_dir_all(&dir);
    }
}
