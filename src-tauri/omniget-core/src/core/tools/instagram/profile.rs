...
fn parse_user_info(v: &Value) -> UserInfo {
    let hd = v
        ...
        .and_then(|x| x.as_str())
        .map(|x| x.to_string())
        .or_else(|| v.get("hd_profile_pic_versions").and_then(|a| a.as_array()).and_then(|a| a.last()).and_then(|x| x.get("url")).and_then(|x| x.as_str()).map(|x| x.to_string()))
        .or_else(|| v.get("profile_pic_url_hd").and_then(|x| x.as_str()).map(|x| x.to_string()))
        .unwrap_or_else(|| s(v, "profile_pic_url"));
    let count = |k: &str, edge: &str| {
        let n = u(v, k);
        if n > 0 { n } else { v.get(edge).and_then(|e| e.get("count")).and_then(|c| c.as_u64()).unwrap_or(0) }
    };
    let pk = {
        let p = s(v, "pk");
    ...
    UserInfo {
        ...
        category: {
            let c = s(v, "category");
            if c.is_empty() { s(v, "category_name") } else { c }
        },
        profile_pic_url: s(v, "profile_pic_url"),
        profile_pic_hd: hd,
        followed_by_viewer: b(v, "followed_by_viewer") || v.get("friendship_status").map(|f| b(f, "following")).unwrap_or(false),
        follows_viewer: b(v, "follows_viewer") || v.get("friendship_status").map(|f| b(f, "followed_by")).unwrap_or(false),
...
pub async fn user_by_id(client: &IgClient, id: &str) -> Result<UserInfo, IgError> {
    let json = client.get_json(&format!("/api/v1/users/{}/info/", id), &[]).await?;
    let user = json.get("user").ok_or_else(|| IgError::NotFound(format!("usuario {}", id)))?;
...
pub async fn resolve_user(client: &IgClient, name_or_id: &str) -> Result<UserInfo, IgError> {
    ...
    if let Ok(json) = client.get_json("/api/v1/users/web_profile_info/", &[("username", name.clone())]).await {
        if let Some(user) = json.get("data").and_then(|d| d.get("user")).filter(|u| !u.is_null()) {
            let mut info = parse_user_info(user);
    ...
    let json = client.get_json("/api/v1/web/search/topsearch/", &[("query", name.clone()), ("context", "blended".into())]).await?;
...
pub async fn user_posts(client: &IgClient, username: &str, limit: usize, flag: &AtomicBool, progress: &super::super::ProgressFn, job: &str) -> Result<Vec<MediaItem>, IgError> {
    ...
    loop {
        let (name, vars) = if first {
            ("PolarisProfilePostsQuery", json!({"data": data, "username": username}))
        } else {
            ("PolarisProfilePostsTabContentQuery_connection", json!({"after": after, "before": null, "data": data, "first": 12, "last": null, "username": username}))
        };
        let json = client.graphql(name, vars).await?;
        let conn = json.get("data").and_then(|d| d.get("xdt_api__v1__feed__user_timeline_graphql_connection")).cloned().unwrap_or(Value::Null);
        if conn.is_null() {
            if first {
        ...
        let edges = conn.get("edges").and_then(|e| e.as_array()).cloned().unwrap_or_default();
        for e in &edges {
            if let Some(item) = e.get("node").and_then(parse_item) {
                out.push(item);
            }
        }
        super::super::report(progress, &id, "list", out.len() as u64, if limit > 0 { Some(limit as u64) } else { None }, None);
        let has_next = conn.get("page_info").map(|p| b(p, "has_next_page")).unwrap_or(false);
        after = conn.get("page_info").and_then(|p| p.get("end_cursor")).and_then(|c| c.as_str()).map(|c| c.to_string());
        first = false;
        if edges.is_empty() || !has_next || after.is_none() || (limit > 0 && out.len() >= limit) || !page_pause_check(flag) {
            break;
        }
        client.pause().await;
    }
...
pub async fn user_reels(client: &IgClient, user_id: &str, limit: usize, flag: &AtomicBool, progress: &super::super::ProgressFn, job: &str) -> Result<Vec<MediaItem>, IgError> {
    let mut out = Vec::new();
    let mut max_id: Option<String> = None;
    let id = format!("ig:{}", job);
    loop {
        let mut form = vec![("target_user_id", user_id.to_string()), ("page_size", "12".to_string()), ("include_feed_video", "true".to_string())];
        ...
        max_id = paging.get("max_id").and_then(|m| m.as_str()).map(|m| m.to_string());
        if items.is_empty() || !b(&paging, "more_available") || max_id.is_none() || (limit > 0 && out.len() >= limit) || !page_pause_check(flag) {
            break;
        }
        client.pause().await;
    }
...
async fn rest_feed(client: &IgClient, path: &str, count: u32, limit: usize, flag: &AtomicBool, progress: &super::super::ProgressFn, job: &str, wrapped: bool) -> Result<Vec<MediaItem>, IgError> {
    let mut out = Vec::new();
    let mut max_id: Option<String> = None;
    let id = format!("ig:{}", job);
    loop {
        ...
        let items = json.get("items").and_then(|i| i.as_array()).cloned().unwrap_or_default();
        for it in &items {
            let node = if wrapped { it.get("media").unwrap_or(it) } else { it };
        ...
        if items.is_empty() || !b(&json, "more_available") || max_id.is_none() || (limit > 0 && out.len() >= limit) || !page_pause_check(flag) {
            break;
        }
        client.pause().await;
    }
...
pub async fn user_tagged(client: &IgClient, user_id: &str, limit: usize, flag: &AtomicBool, progress: &super::super::ProgressFn, job: &str) -> Result<Vec<MediaItem>, IgError> {
    rest_feed(client, &format!("/api/v1/usertags/{}/feed/", user_id), 20, limit, flag, progress, job, false).await
}

/// Posts salvos da própria conta (todos, sem separar por coleção).
pub async fn saved(client: &IgClient, limit: usize, flag: &AtomicBool, progress: &super::super::ProgressFn, job: &str) -> Result<Vec<MediaItem>, IgError> {
    rest_feed(client, "/api/v1/feed/saved/posts/", 30, limit, flag, progress, job, true).await
}
...
pub async fn highlights(client: &IgClient, user_id: &str) -> Result<Vec<Highlight>, IgError> {
    let json = client.get_json(&format!("/api/v1/highlights/{}/highlights_tray/", user_id), &[]).await?;
    let tray = json.get("tray").and_then(|t| t.as_array()).cloned().unwrap_or_default();
    Ok(tray
        .iter()
        .map(|h| Highlight {
            ...
            cover: h
                .get("cover_media")
                .and_then(|c| c.get("cropped_image_version").or(c.get("full_image_version")))
...
pub async fn reels_media(client: &IgClient, reel_ids: &[String]) -> Result<Vec<Reel>, IgError> {
    ...
    let reels = json.get("reels_media").and_then(|r| r.as_array()).cloned().unwrap_or_default();
    let mut out = Vec::new();
    for reel in reels {
        let user = reel.get("user").cloned().unwrap_or(Value::Null);
        let username = s(&user, "username");
        let title = reel.get("title").and_then(|t| t.as_str()).map(|t| t.to_string());
        let mut items = Vec::new();
        for it in reel.get("items").and_then(|i| i.as_array()).cloned().unwrap_or_default() {
        ...
        let close_friends = reel.get("items").and_then(|i| i.as_array()).map(|a| a.iter().any(|x| s(x, "audience") == "besties")).unwrap_or(false);
        out.push(Reel {
            id: s(&reel, "id"),
            title,
            username,
...
pub async fn highlight_items(client: &IgClient, highlight_id: &str) -> Result<Vec<MediaItem>, IgError> {
    let id = if highlight_id.starts_with("highlight:") { highlight_id.to_string() } else { format!("highlight:{}", highlight_id) };
    ...
    Ok(tray
        ...
        .map(|t| {
            ...
            TrayEntry {
                ...
                latest_reel_media: t.get("latest_reel_media").and_then(|x| x.as_i64()).unwrap_or(0),
...
pub async fn story_viewers(client: &IgClient, story_pk: &str, flag: &AtomicBool) -> Result<(u64, Vec<super::follow::MiniUser>), IgError> {
    let mut out = Vec::new();
    let mut max_id: Option<String> = None;
    let mut total = 0u64;
    loop {
        ...
        let json = client.get_json(&format!("/api/v1/media/{}/list_reel_media_viewer/", story_pk), &q).await?;
        total = total.max(u(&json, "total_viewer_count"));
        let users = json.get("users").and_then(|u| u.as_array()).cloned().unwrap_or_default();
        for us in &users {
            out.push(super::follow::MiniUser::from_value(us));
        }
        max_id = json.get("next_max_id").and_then(|m| m.as_str()).map(|m| m.to_string());
...
pub async fn friendship(client: &IgClient, user_id: &str) -> Result<Friendship, IgError> {
    let v = client.get_json(&format!("/api/v1/friendships/show/{}/", user_id), &[]).await?;
    Ok(Friendship {
