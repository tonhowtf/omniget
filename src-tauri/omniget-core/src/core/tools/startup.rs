...
async fn plist_json(path: &Path) -> Option<serde_json::Value> {
    let o = crate::core::process::command("plutil").args(["-convert", "json", "-o", "-"]).arg(path).output().await.ok()?;
    if !o.status.success() {
        return None;
    }
...
async fn mac_items() -> Vec<StartupItem> {
    let mut out = Vec::new();
    let uid = run("id", &["-u"]).await.map(|s| s.trim().to_string()).unwrap_or_else(|_| "501".into());
    let disabled = run("launchctl", &["print-disabled", &format!("gui/{}", uid)]).await.unwrap_or_default();
    for (dir, scope) in [(home().join("Library/LaunchAgents"), "user"), (PathBuf::from("/Library/LaunchAgents"), "system")] {
        let Ok(rd) = std::fs::read_dir(&dir) else { continue };
        let mut entries: Vec<_> = rd.flatten().map(|e| e.path()).filter(|p| p.extension().map(|e| e == "plist").unwrap_or(false)).collect();
        entries.sort();
        for p in entries {
            let json = plist_json(&p).await.unwrap_or(serde_json::Value::Null);
            let label = json.get("Label").and_then(|v| v.as_str()).map(String::from).unwrap_or_else(|| p.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default());
            let command = json
                .get("ProgramArguments")
                .and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|x| x.as_str()).collect::<Vec<_>>().join(" "))
                .or_else(|| json.get("Program").and_then(|v| v.as_str()).map(String::from))
                .unwrap_or_default();
            let disabled_key = json.get("Disabled").and_then(|v| v.as_bool()).unwrap_or(false);
            let disabled_override = disabled.lines().any(|l| l.contains(&format!("\"{}\" => disabled", label)) || l.contains(&format!("\"{}\" => true", label)));
    ...
    if let Ok(list) = run("osascript", &["-e", "tell application \"System Events\" to get {name, path, hidden} of every login item"]).await {
        ...
        let parts: Vec<Vec<String>> = list
            ...
            .trim_end_matches('}')
            .split("}, {")
            .map(|g| g.trim_matches(|c| c == '{' || c == '}').split(", ").map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect())
            .collect();
        if parts.len() >= 2 {
    ...
    for (name, path) in removed_login_items() {
        if !out.iter().any(|i| i.source == "login-item" && i.name == name) {
            out.push(StartupItem { id: format!("li:{}", name), name, command: path.clone(), source: "login-item".into(), scope: "user".into(), path, enabled: false, can_toggle: true });
        }
    }
    out
}
...
fn removed_login_items() -> Vec<(String, String)> {
    removed_file().and_then(|p| std::fs::read_to_string(p).ok()).and_then(|s| serde_json::from_str(&s).ok()).unwrap_or_default()
}
...
async fn mac_set(item: &StartupItem, enabled: bool) -> anyhow::Result<()> {
    match item.source.as_str() {
        "launch-agent" => {
            ...
            if enabled {
                let _ = run("launchctl", &["enable", &format!("{}/{}", target, item.name)]).await;
                run("launchctl", &["bootstrap", &target, &item.path]).await.or_else(|_| Ok::<_, anyhow::Error>(String::new()))?;
            } else {
                let _ = run("launchctl", &["bootout", &format!("{}/{}", target, item.name)]).await;
                run("launchctl", &["disable", &format!("{}/{}", target, item.name)]).await?;
            }
            Ok(())
        }
        "login-item" => {
            ...
            if enabled {
                let path = removed.iter().find(|(n, _)| *n == item.name).map(|(_, p)| p.clone()).unwrap_or_else(|| item.path.clone());
                if path.is_empty() {
                ...
                run("osascript", &["-e", &format!("tell application \"System Events\" to delete login item \"{}\"", item.name.replace('"', "\\\""))]).await?;
                removed.retain(|(n, _)| *n != item.name);
...
const WIN_RUN: &[(&str, &str, &str)] = &[
    ...
    (r"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "run", "system"),
    (r"HKLM\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run", "run", "system"),
    (r"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce", "run-once", "system"),
];

fn approved_key(scope: &str, folder: bool) -> String {
    let root = if scope == "user" { "HKCU" } else { "HKLM" };
    format!(r"{}\Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\{}", root, if folder { "StartupFolder" } else { "Run" })
}
...
async fn win_approved(scope: &str, folder: bool) -> std::collections::HashMap<String, bool> {
    ...
    for key in super::win_registry::reg_query(&approved_key(scope, folder), false).await {
        for v in key.values {
            ...
            let enabled = v.data.get(0..2).map(|b| b == "02" || b == "06").unwrap_or(true);
...
async fn win_items() -> Vec<StartupItem> {
    ...
    for (dir, scope) in [
        (std::env::var("APPDATA").map(|a| PathBuf::from(a).join(r"Microsoft\Windows\Start Menu\Programs\Startup")), "user"),
        (std::env::var("ProgramData").map(|a| PathBuf::from(a).join(r"Microsoft\Windows\Start Menu\Programs\StartUp")), "system"),
    ] {
...
async fn win_set(item: &StartupItem, enabled: bool) -> anyhow::Result<()> {
    ...
    let name = if folder { Path::new(&item.path).file_name().map(|s| s.to_string_lossy().to_string()).unwrap_or_default() } else { item.name.clone() };
    let data = if enabled { "020000000000000000000000" } else { "030000000000000000000000" };
...
fn desktop_field(text: &str, key: &str) -> Option<String> {
    text.lines().find_map(|l| l.strip_prefix(&format!("{}=", key)).map(|v| v.trim().to_string()))
}

async fn linux_items() -> Vec<StartupItem> {
    let mut out = Vec::new();
    let user_dir = dirs::config_dir().unwrap_or_else(|| home().join(".config")).join("autostart");
    let mut seen = std::collections::HashSet::new();
    for (dir, scope) in [(user_dir.clone(), "user"), (PathBuf::from("/etc/xdg/autostart"), "system")] {
        let Ok(rd) = std::fs::read_dir(&dir) else { continue };
        let mut entries: Vec<_> = rd.flatten().map(|e| e.path()).filter(|p| p.extension().map(|e| e == "desktop").unwrap_or(false)).collect();
        entries.sort();
        for p in entries {
            ...
            let hidden = desktop_field(&text, "Hidden").map(|v| v == "true").unwrap_or(false);
            let gnome_off = desktop_field(&text, "X-GNOME-Autostart-enabled").map(|v| v == "false").unwrap_or(false);
            out.push(StartupItem {
                id: format!("autostart:{}", file),
                name: desktop_field(&text, "Name").unwrap_or_else(|| file.trim_end_matches(".desktop").to_string()),
    ...
    if let Ok(list) = run("systemctl", &["--user", "list-unit-files", "--type=service", "--no-legend", "--no-pager"]).await {
        for l in list.lines() {
            let mut cols = l.split_whitespace();
            let (Some(unit), Some(state)) = (cols.next(), cols.next()) else { continue };
...
async fn linux_set(item: &StartupItem, enabled: bool) -> anyhow::Result<()> {
    match item.source.as_str() {
        "autostart" => {
            ...
            let file = Path::new(&item.path).file_name().ok_or_else(|| anyhow!("arquivo invalido"))?;
            let target = user_dir.join(file);
            let mut text = std::fs::read_to_string(&item.path).unwrap_or_default();
            let mut lines: Vec<String> = text.lines().filter(|l| !l.starts_with("Hidden=") && !l.starts_with("X-GNOME-Autostart-enabled=")).map(String::from).collect();
            if !enabled {
        ...
        "systemd-user" => {
            run("systemctl", &["--user", if enabled { "enable" } else { "disable" }, &item.path]).await?;
            Ok(())
        }
        _ => Err(anyhow!("nao da para alterar este item")),
    }
...
pub async fn set_enabled(item: &StartupItem, enabled: bool) -> anyhow::Result<()> {
    if !item.can_toggle {
        return Err(anyhow!("este item so pode ser alterado com privilegios de administrador"));
    }
...
mod tests {
    ...
    fn desktop() {
        ...
        assert_eq!(desktop_field(t, "Exec").as_deref(), Some("/usr/bin/foo --x"));
...
mod live {
    ...
    async fn list_here() {
        ...
        for i in &items {
            println!("{:<14} {:<6} {:<5} {} :: {}", i.source, i.scope, i.enabled, i.name, i.command.chars().take(80).collect::<String>());
        }
