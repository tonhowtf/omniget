...
impl XClient {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let http = crate::core::http_client::apply_global_proxy(reqwest::Client::builder())
            ...
            .build()?;
        let cookie = crate::platforms::cookie_provider::cookie_header_for_domains(&["x.com", "twitter.com"])
    ...
    pub fn authed(&self) -> bool {
        self.cookie.as_deref().map(|c| cookie_value(c, "auth_token").is_some() && cookie_value(c, "ct0").is_some()).unwrap_or(false)
    }
    ...
    pub fn user_id(&self) -> Option<String> {
        ...
        let dec = urlencoding::decode(&raw).map(|c| c.to_string()).unwrap_or(raw);
        let id = dec.trim().trim_start_matches("u=").trim_matches('"').to_string();
    ...
    async fn guest_token(&self, force: bool) -> anyhow::Result<String> {
        ...
        if !resp.status().is_success() {
            return Err(anyhow!("X nao entregou guest token: HTTP {}", resp.status()));
        }
        let v: Value = resp.json().await?;
        let tok = v.get("guest_token").and_then(|t| t.as_str()).ok_or_else(|| anyhow!("guest token ausente"))?.to_string();
    ...
    async fn tx_header(&self, method: &str, path: &str) -> Option<String> {
        ...
        slot.as_ref().and_then(|(_, g, _)| g.as_ref()).map(|g| g.calc(method, path))
    }
    ...
    async fn headers(&self, method: &str, path: &str) -> anyhow::Result<HeaderMap> {
        ...
        if self.authed() {
            h.insert("Cookie", HeaderValue::from_str(self.cookie.as_deref().unwrap_or(""))?);
            ...
            h.insert("Cookie", HeaderValue::from_str(&format!("guest_id=v1%3A{}", tok))?);
        }
        Ok(h)
    }
    ...
    async fn check(resp: reqwest::Response, op: &str) -> anyhow::Result<Result<Value, String>> {
        ...
        if !status.is_success() {
            return Err(anyhow!("X {}: HTTP {} {}", op, status, text.chars().take(200).collect::<String>()));
        }
        let v: Value = serde_json::from_str(&text).map_err(|e| anyhow!("X {}: resposta invalida ({})", op, e))?;
        if v.get("data").map(|d| d.is_null() || d.as_object().map(|o| o.is_empty()).unwrap_or(false)).unwrap_or(true) {
            if let Some(msg) = v.get("errors").and_then(|e| e.as_array()).and_then(|a| a.first()).and_then(|e| e.get("message")).and_then(|m| m.as_str()) {
    ...
    async fn refresh_ids(&self) -> anyhow::Result<()> {
        super::query_ids::refresh(&self.http, self.cookie.as_deref()).await.map(|_| ())
    }

    pub async fn gql_get(&self, op: &str, variables: Value, extra_features: Value, field_toggles: Option<Value>) -> anyhow::Result<Value> {
        ...
        loop {
            tries += 1;
            let id = super::query_ids::id_for(op).ok_or_else(|| anyhow!("operacao {} desconhecida", op))?;
            let (url, path) = self.gql_url(&id, op);
            let mut query: Vec<(&str, String)> = vec![("variables", variables.to_string()), ("features", features.to_string())];
            if let Some(ft) = &field_toggles {
                query.push(("fieldToggles", ft.to_string()));
            }
            let headers = self.headers("GET", &path).await?;
            let resp = self.http.get(&url).headers(headers).query(&query).send().await?;
            match Self::check(resp, op).await? {
                ...
                Err(kind) if tries < 3 => {
                    if kind == "not_found" {
                        ...
                        return Err(anyhow!("X {}: HTTP {} (sessao expirada? entre de novo no X)", op, kind.trim_start_matches("auth:")));
                    }
                }
    ...
    pub async fn gql_post(&self, op: &str, variables: Value, features: Option<Value>) -> anyhow::Result<Value> {
        self.require_login()?;
        let mut tries = 0;
        loop {
            tries += 1;
    ...
    pub async fn post_json_raw(&self, url: &str, body: &Value, extra: &[(&str, &str)]) -> anyhow::Result<reqwest::Response> {
        self.require_login()?;
        ...
        if !resp.status().is_success() {
            let st = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow!("X: HTTP {} {}", st, text.chars().take(300).collect::<String>()));
        }
        Ok(resp)
    }
    ...
    pub async fn paginate<F>(&self, op: &str, mut variables: Value, extra_features: Value, limit: usize, job: &str, mut on_page: F) -> anyhow::Result<usize>
    where
        F: FnMut(&Value) -> usize,
    {
        ...
        loop {
            ...
            let page = self.gql_get(op, variables.clone(), extra_features.clone(), None).await?;
            ...
            match next {
                Some(c) if Some(&c) != cursor.as_ref() && empty < 3 && !stalled && (limit == 0 || total < limit) => cursor = Some(c),
                _ => break,
            }
