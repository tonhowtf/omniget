...
impl Source {
    fn open(path: &str) -> anyhow::Result<Self> {
        ...
        if p.is_dir() {
            let data = if p.join("data").is_dir() {
                p.join("data")
            } else {
                p.to_path_buf()
            };
    ...
    fn json(&self, base: &str) -> Vec<Value> {
        let mut names: Vec<&String> = self
            .files
            .keys()
            .filter(|n| *n == &format!("{}.js", base) || n.starts_with(&format!("{}-part", base)))
            .collect();
        names.sort();
        let mut out = Vec::new();
        for n in names {
            ...
            if let Ok(Value::Array(a)) =
                serde_json::from_str::<Value>(s[i + 1..].trim().trim_end_matches(';'))
            {
                out.extend(a);
            }
        }
...
fn account_list(items: &[Value], key: &str) -> Vec<ArchiveAccount> {
    items
        ...
        .map(|a| ArchiveAccount {
            account_id: s(a, "accountId"),
            user_link: s(a, "userLink"),
        })
...
fn tweet_from(v: &Value, author: &XUser) -> Option<XPost> {
    ...
    Some(XPost {
        ...
        reposted_by: if is_rt {
            Some(author.handle.clone())
        } else {
            None
        },
        source: regex::Regex::new(r"<[^>]+>")
            .unwrap()
            .replace_all(&s(t, "source"), "")
            .to_string(),
        links,
        ..Default::default()
    })
...
pub fn load(path: &str) -> anyhow::Result<Loaded> {
    ...
    let likes: Vec<LikeItem> = src
        ...
        .map(|l| LikeItem {
            tweet_id: s(l, "tweetId"),
            text: s(l, "fullText"),
            url: if s(l, "expandedUrl").is_empty() {
                format!("https://x.com/i/status/{}", s(l, "tweetId"))
            } else {
                s(l, "expandedUrl")
            },
        })
    ...
    let follower_ids: std::collections::HashSet<&str> =
        followers.iter().map(|a| a.account_id.as_str()).collect();
    let following_ids: std::collections::HashSet<&str> =
        following.iter().map(|a| a.account_id.as_str()).collect();
    let not_following_back: Vec<ArchiveAccount> = following
        .iter()
        .filter(|a| !follower_ids.contains(a.account_id.as_str()))
        .cloned()
        .collect();
    let fans: Vec<ArchiveAccount> = followers
        .iter()
        .filter(|a| !following_ids.contains(a.account_id.as_str()))
        .cloned()
        .collect();
    ...
    for p in &posts {
        ...
        if let Some(dt) = chrono::Local.timestamp_opt(p.timestamp, 0).single() {
            let e = by_year.entry(dt.year()).or_insert(YearCount {
                year: dt.year(),
                ..Default::default()
            });
            e.tweets += 1;
    ...
    let mut top: Vec<XPost> = posts
        .iter()
        .filter(|p| p.reposted_by.is_none())
        .cloned()
        .collect();
    ...
    let summary = ArchiveSummary {
        ...
        muted: muted.len(),
        dm_messages,
        first_tweet: if min_ts < i64::MAX {
            super::iso_from_timestamp(min_ts)
        } else {
            String::new()
        },
        last_tweet: if max_ts > i64::MIN {
...
pub fn export(path: &str, dest_dir: &str, what: &str, format: &str) -> anyhow::Result<String> {
    ...
    let accounts = |list: &[ArchiveAccount]| -> anyhow::Result<String> {
        let content = match format {
            ...
            "csv" => {
                ...
                for a in list {
                    o.push_str(&format!(
                        "{},{}\n",
                        a.account_id,
                        if a.user_link.is_empty() {
                            format!("https://x.com/i/user/{}", a.account_id)
                        } else {
                            a.user_link.clone()
                        }
                    ));
            ...
            _ => {
                list.iter()
                    .map(|a| format!("- https://x.com/i/user/{}", a.account_id))
                    .collect::<Vec<_>>()
                    .join("\n")
                    + "\n"
            }
    ...
    match what {
        "tweets" => super::export::write_posts(
            &l.posts,
            format,
            &file,
            &format!("Posts de @{}", l.summary.username),
        ),
        "likes" => {
            let content = match format {
                ...
                "csv" => {
                    ...
                    for k in &l.likes {
                        o.push_str(&format!(
                            ...
                            k.tweet_id,
                            k.url,
                            super::export::csv_escape(&k.text)
                        ));
                    }
                    o
                }
