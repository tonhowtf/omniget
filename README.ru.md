...
- [Для игроков League of Legends, выключено по умолчанию](#для-игроков-league-of-legends-выключено-по-умолчанию)
...
Страница «Загрузки» показывает скорость, этап и оставшееся время прямо от загрузчика, поэтому зависшая загрузка выглядит зависшей, а не застывшей на «осталось 3 секунды». Прерванные загрузки продолжаются с места остановки. Сайты с ограничением запросов повторяются с backoff.
...
|---|---|
| Онлайн-курсы | Hotmart, Udemy, Kiwify и Rocketseat через плагин Courses. Все уроки, выбор разделов, вложения, продолжение с места остановки. |
...
Расширение делает две вещи. На сайтах, которые оно знает (YouTube, Instagram, TikTok, X, Reddit, Twitch, Pinterest, Bluesky, Telegram, Vimeo, Udemy, Hotmart, Bilibili, SoundCloud), оно отправляет страницу в OmniGet одним кликом или по **Alt+O**. На любом другом сайте оно следит за сетевым трафиком, находит потоки MP4, HLS, DASH, WebM и аудио и показывает их во всплывающем окне. В обоих случаях оно передаёт ваши cookies и referer, и именно это позволяет OmniGet скачивать приватный контент, в который вы вошли: сторис Instagram, купленный курс, видео только для участников.
...
Tools — та часть OmniGet, которая выросла за пределы загрузки. Каждая плитка — одна задача: изолированная Rust-команда с JSON на входе и JSON на выходе, что в будущем позволит ИИ-агентам управлять инструментами через запланированный MCP-сервер. В хабе есть поиск, понимающий английский и португальский («legenda» находит инструменты для субтитров), и фильтр по платформам; инструменты только для Windows говорят об этом на плитке и скрыты на других системах.
...
- **Принудительный H.264.** Избегать VP9 и AV1 на слабых машинах. *в планах*
...
- **Клонировать голос**, **Создать голос**, **Выделить вокал**, **Диктовка.** *в планах*
...
- **Запись экрана** и **Редактор с таймлайном.** *в планах*
...
- **Очистка кэшей**, **Анализ диска**, **Автозагрузка**, **Деинсталлятор**, **Debloat Windows**, **Чистка реестра**, **Обновление программ** (winget, Chocolatey и Scoop). *в планах*

### Автоматизация (1)

- **Автокликер.** Windows. *в планах*
...
- **API-ключи.** Храните ключи и аккаунты вместе, проверяйте баланс и использование. *в планах*
- **MCP-сервер.** Позвольте Claude, Goose или Cursor управлять инструментами OmniGet. *в планах*

Каждый инструмент, работающий с ИИ, использует провайдера из **Настройки → ИИ**: OpenAI, Anthropic или любую OpenAI-совместимую локальную точку, например Ollama или LM Studio. Ключ хранится локально и никогда не логируется.
...
Войдите в **Hotmart**, **Udemy**, **Kiwify** или **Rocketseat** через окно браузера внутри приложения или с cookies, сохранёнными расширением. OmniGet покажет ваши покупки, позволит выбрать разделы и скачает все уроки и вложения, при желании со сквозной нумерацией. Уроки с DRM подсчитываются и пропускаются, а не имитируются. Hotmart использует актуальный вход через OIDC, так что всё работает после смены авторизации в 2026 году. Скачанные курсы автоматически появляются в Study.
...
## Для игроков League of Legends, выключено по умолчанию

Включите в **Настройки → Дополнительно → League of Legends**, и появится меню League. Оно читает запущенный клиент League локально, без аккаунта и без сторонних сайтов с билдами.
...
Да. Установите плагин Courses (он предустановлен), войдите через приложение, выберите курс и разделы и скачивайте. Уроки и вложения лягут в папку курса и появятся в Study.
