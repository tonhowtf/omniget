...
copy_tree() {
    ...
    for elem in "${src[@]}"; do
        # Some distros (e.g. Arch with builtin gdk-pixbuf loaders) ship stale
        # pkg-config paths; skip missing sources instead of failing the build.
        if [ ! -e "$elem" ]; then
            echo "WARNING: copy_tree: '$elem' does not exist, skipping"
            continue
        fi
        mkdir -p "${dst::-1}$elem"
